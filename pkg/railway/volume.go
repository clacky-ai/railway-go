...
func (c *Client) DeleteVolumeBackups(ctx context.Context, volumeInstanceID string, volumeInstanceBackupIDs []string) (string, error) {
	var resp igql.VolumeInstanceBackupBatchDeleteResponse
	if err := c.gqlClient.MutateInternal(ctx, igql.VolumeInstanceBackupBatchDeleteMutation, map[string]any{
		"volumeInstanceId":        volumeInstanceID,
		"volumeInstanceBackupIds": volumeInstanceBackupIDs,
	}, &resp); err != nil {
		return "", err
	}
	return resp.VolumeInstanceBackupBatchDelete.WorkflowID, nil
}
